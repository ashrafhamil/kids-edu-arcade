...
export default function Game() {
  ...
  function handleTimeout(): void {
    if (phase !== "playing" || !round || resolving.current) return;
    resolving.current = true;
    registerMiss(score, null, round.correct.name);
  }
    ...
    <GameShell meta={meta} right={liveStats}>
      ...
      {phase === "playing" && round && (
        <div className="flex w-full flex-col items-center gap-4">
          ...
          >
            <span className="text-8xl leading-none drop-shadow" aria-hidden>
              {round.correct.emoji}
            </span>
          </div>

          <div className="text-2xl font-black drop-shadow">How does this feel?</div>
...
function ChoiceTile({
  ...
  return (
    <button
      ...
      disabled={disabled}
      aria-label={name}
      className={`relative flex min-h-16 w-36 select-none items-center justify-center rounded-2xl bg-white px-3 py-4 text-slate-900 shadow-lg shadow-black/20 transition active:scale-95 disabled:active:scale-100 ${feedback}`}
    >
...
function ReadyPanel({ onPlay }: { onPlay: () => void }) {
  return (
    <Panel>
      ...
      <h2 className="mt-2 text-3xl font-black text-slate-800">Feelings</h2>
      <p className="mt-2 text-base font-semibold text-slate-600">
        A face pops up — tap the word that names how it feels: Happy, Sad, Angry, Surprised
        and more!
      </p>
...
function OverPanel({
  ...
  return (
    <Panel>
      ...
      <div className="mt-3 text-5xl font-black tabular-nums text-fuchsia-600">{score}</div>
