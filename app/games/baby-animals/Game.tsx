...
export default function Game() {
  ...
  function registerMiss(currentScore: number, choiceId: string | null, prevAnswer: string): void {
    sfx.wrong();
    setCombo(0);
    setWrongId(choiceId);
  ...
  function handleChoice(choiceId: string): void {
    ...
    if (choiceId !== round.answer) {
      registerMiss(score, choiceId, round.answer);
      return;
    }
  ...
  function choiceVisual(id: string): ChoiceVisual {
    if (revealAnswer && round && id === round.answer) return "correct";
    ...
    <GameShell meta={meta} right={liveStats}>
      ...
      {phase === "playing" && round && (
        <div className="flex w-full flex-col items-center gap-4">
          ...
          <div className="text-2xl font-black drop-shadow">Tap its baby!</div>
          ...
          <div className="relative flex w-full max-w-sm flex-wrap items-stretch justify-center gap-3">
            {round.choices.map((choice, index) => (
              ...
              >
                <ChoiceTile
                  emoji={choice.babyEmoji}
                  label={choice.babyName}
                  visual={choiceVisual(choice.id)}
                  disabled={revealAnswer}
                  onTap={() => handleChoice(choice.id)}
                />
                {floatGain > 0 && round.answer === choice.id && (
                  <span
                    key={floatKey}
...
function ChoiceTile({
  ...
  return (
    <button
      ...
      disabled={disabled}
      aria-label={label}
      className={`relative flex min-h-24 w-32 select-none items-center justify-center rounded-2xl bg-white px-2 py-3 text-slate-900 shadow-lg shadow-black/20 transition active:scale-95 disabled:active:scale-100 ${feedback}`}
    >
...
function ReadyPanel({ onPlay }: { onPlay: () => void }) {
  return (
    <Panel>
      ...
      <h2 className="mt-2 text-3xl font-black text-slate-800">Baby Animals</h2>
      <p className="mt-2 text-base font-semibold text-slate-600">
        A grown-up animal pops up — tap the little baby that belongs to it!
      </p>
