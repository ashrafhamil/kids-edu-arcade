...
import { genRound, starsFor, choiceCountFor, type PartId, type Round } from "./rounds";
import PlantSvg from "./PlantSvg";
import TimerBar from "./TimerBar";

const SLUG = "plant-parts";
...
export default function Game() {
  ...
  const [wrongId, setWrongId] = useState<PartId | null>(null);
  ...
  function loadNext(forScore: number, avoidId: PartId): void {
    present(genRound(nextId.current++, forScore, avoidId));
  }
  ...
  function registerMiss(currentScore: number, choiceId: PartId | null, prevAnswer: PartId): void {
    sfx.wrong();
    setCombo(0);
    setWrongId(choiceId);
  ...
  function handleChoice(choiceId: PartId): void {
    if (phase !== "playing" || !round || resolving.current) return;
    resolving.current = true;

    if (choiceId !== round.correct.id) {
      registerMiss(score, choiceId, round.correct.id);
      return;
    }
    ...
    setFloatGain(points);
    setFloatKey((k) => k + 1);
    schedule(() => loadNext(nextScore, round.correct.id), CORRECT_DELAY);
  }

  function handleTimeout(): void {
    if (phase !== "playing" || !round || resolving.current) return;
    resolving.current = true;
    registerMiss(score, null, round.correct.id);
  }

  function choiceVisual(id: PartId): ChoiceVisual {
    if (revealAnswer && round && id === round.correct.id) return "correct";
    ...
    <GameShell meta={meta} right={liveStats}>
      ...
      {phase === "playing" && round && (
        <div className="flex w-full flex-col items-center gap-3">
          ...
          {/* The plant itself never remounts — only the glowing part changes. */}
          <div className="w-full max-w-[13.5rem] rounded-3xl bg-white/95 p-3 shadow-2xl shadow-black/30">
            <PlantSvg highlight={round.correct.id} />
          </div>

          <div key={round.id} className="animate-pop-in text-2xl font-black drop-shadow">
            Which part is glowing?
          </div>
          ...
          <div className="relative flex w-full max-w-sm flex-wrap items-stretch justify-center gap-3">
            {round.choices.map((choice, index) => (
              ...
              >
                ...
                />
                {floatGain > 0 && round.correct.id === choice.id && (
                  <span
                    key={floatKey}
...
function ChoiceTile({
  ...
  return (
    <button
      ...
      disabled={disabled}
      aria-label={name}
      className={`relative flex min-h-16 w-32 select-none flex-col items-center justify-center gap-0.5 rounded-2xl bg-white px-2 py-2 text-slate-900 shadow-lg shadow-black/20 transition active:scale-95 disabled:active:scale-100 ${feedback}`}
    >
...
function ReadyPanel({ onPlay }: { onPlay: () => void }) {
  return (
    <Panel>
      ...
      <h2 className="mt-2 text-3xl font-black text-slate-800">Plant Parts</h2>
      <p className="mt-2 text-base font-semibold text-slate-600">
        One part of the plant lights up — tap its name: root, stem, leaf, flower, fruit or
        seed!
      </p>
...
function OverPanel({
  ...
  return (
    <Panel>
      ...
      <div className="mt-3 text-5xl font-black tabular-nums text-emerald-700">{score}</div>
