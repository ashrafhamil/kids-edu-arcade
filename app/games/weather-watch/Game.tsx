...
export default function Game() {
  ...
  function handleChoice(choiceName: string): void {
    ...
    if (choiceName !== round.answer) {
      registerMiss(score, choiceName, round.answer);
      return;
    }
  ...
  function choiceVisual(name: string): ChoiceVisual {
    if (revealAnswer && round && name === round.answer) return "correct";
          ...
          <div className="relative flex w-full max-w-sm flex-wrap items-stretch justify-center gap-3">
            {round.choices.map((choice, index) => (
              ...
              >
                ...
                />
                {floatGain > 0 && round.answer === choice.name && (
                  <span
                    key={floatKey}
...
function ReadyPanel({ onPlay }: { onPlay: () => void }) {
  return (
    <Panel>
      ...
      <h2 className="mt-2 text-3xl font-black text-slate-800">Weather Watch</h2>
      <p className="mt-2 text-base font-semibold text-slate-600">
        A scene pops up — tap the weather it shows: Sunny, Rainy, Snowy, Windy, Cloudy or
        Stormy!
      </p>
